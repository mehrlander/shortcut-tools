...
<!-- doc-index -->
**50 sections.** Generated by `python3 tools/doc-index.py --publish`.

- [Control flow is three actions sharing a UUID](#control-flow-is-three-actions-sharing-a-uuid)
  - [The consequence for this repo](#the-consequence-for-this-repo)
- [Variable references bind by producing UUID](#variable-references-bind-by-producing-uuid)
- [Two attachment forms, and the aggrandizements](#two-attachment-forms-and-the-aggrandizements)
- [Run Shortcut names its target twice, and the second half is optional](#run-shortcut-names-its-target-twice-and-the-second-half-is-optional)
- [An `If` with several conditions uses a different shape entirely](#an-if-with-several-conditions-uses-a-different-shape-entirely)
- [`Show-Html`: what it does to a page on the way through](#show-html-what-it-does-to-a-page-on-the-way-through)
- [Parameter shapes, by census](#parameter-shapes-by-census)
- [The token-injection pattern](#the-token-injection-pattern)
- [Injection reaches a compressed page, through the wrapper rather than into it](#injection-reaches-a-compressed-page-through-the-wrapper-rather-than-into-it)
- [Two ways to put HTML on screen, and only one of them keeps you in the shortcut](#two-ways-to-put-html-on-screen-and-only-one-of-them-keeps-you-in-the-shortcut)
- [A menu with icons is a list of contacts, coerced in place](#a-menu-with-icons-is-a-list-of-contacts-coerced-in-place)
- [`Type: "Ask"` is the fourth attachment value](#type-ask-is-the-fourth-attachment-value)
- [Do not index the property names, just compress](#do-not-index-the-property-names-just-compress)
- [A shortcut exports itself through Get File of Type](#a-shortcut-exports-itself-through-get-file-of-type)
- [The library settles four inferred shapes](#the-library-settles-four-inferred-shapes)
- [`Show-Html` and the injector, read rather than described](#show-html-and-the-injector-read-rather-than-described)
  - [The injector is a dictionary lookup against a config file](#the-injector-is-a-dictionary-lookup-against-a-config-file)
- [The device can gzip, and it is one action](#the-device-can-gzip-and-it-is-one-action)
- [Every verb demos itself, and it shows up as a self-call](#every-verb-demos-itself-and-it-shows-up-as-a-self-call)
  - [Condition codes, settled by branch semantics](#condition-codes-settled-by-branch-semantics)
- [The packed route inverts the glyph rule](#the-packed-route-inverts-the-glyph-rule)
- [`actions.json` values are newline-delimited JSON, not JSON](#actionsjson-values-are-newline-delimited-json-not-json)
- [The Run JavaScript action has a performance cliff](#the-run-javascript-action-has-a-performance-cliff)
- [The browser is a coprocessor, not only a destination](#the-browser-is-a-coprocessor-not-only-a-destination)
  - [What it is for](#what-it-is-for)
  - [The coercion route, settled on device 2026-08-28](#the-coercion-route-settled-on-device-2026-08-28)
- [Some actions are load-bearing without appearing in the data flow](#some-actions-are-load-bearing-without-appearing-in-the-data-flow)
- [Values crossing the Shortcuts-to-JavaScript boundary get text-coerced](#values-crossing-the-shortcuts-to-javascript-boundary-get-text-coerced)
- [The data: URL route can reach the network, and the capture moment is the risk](#the-data-url-route-can-reach-the-network-and-the-capture-moment-is-the-risk)
- [A third-party app intent is not a receiver you can build blind](#a-third-party-app-intent-is-not-a-receiver-you-can-build-blind)
- [A list handed to Run Shortcut can arrive text-coerced](#a-list-handed-to-run-shortcut-can-arrive-text-coerced)
- [An inline payload outlives the path it loads from](#an-inline-payload-outlives-the-path-it-loads-from)
- [A vCard menu hides its URLs from a search for them](#a-vcard-menu-hides-its-urls-from-a-search-for-them)
- [A run-shortcut link makes Shortcuts the current app](#a-run-shortcut-link-makes-shortcuts-the-current-app)
- ["Unrecognized archive format" is the signing service, not the file](#unrecognized-archive-format-is-the-signing-service-not-the-file)
- [The library-management actions address an App Intents entity, not a name](#the-library-management-actions-address-an-app-intents-entity-not-a-name)
  - [The best public catalog: shortcuts-playground-plugin's ToolKit dumps](#the-best-public-catalog-shortcuts-playground-plugins-toolkit-dumps)
  - [The device lists apps, and never lists actions](#the-device-lists-apps-and-never-lists-actions)
  - [The `platforms` field is not a runtime claim](#the-platforms-field-is-not-a-runtime-claim)
  - [No action can put actions into a shortcut](#no-action-can-put-actions-into-a-shortcut)
  - [The four sources checked before it, none of them enough](#the-four-sources-checked-before-it-none-of-them-enough)
- [An action that moves the app's UI breaks what runs after it](#an-action-that-moves-the-apps-ui-breaks-what-runs-after-it)
- [A Repeat over names stops at the first name that resolves to nothing](#a-repeat-over-names-stops-at-the-first-name-that-resolves-to-nothing)
- [An installed copy can carry one action the published plist does not](#an-installed-copy-can-carry-one-action-the-published-plist-does-not)
- [A splice that keeps the actions still loses the shortcut's settings](#a-splice-that-keeps-the-actions-still-loses-the-shortcuts-settings)
- [Generating the plist](#generating-the-plist)
- [Related](#related)
- [Checks](#checks)
- [Where the rest of the record is](#where-the-rest-of-the-record-is)
<!-- /doc-index -->
