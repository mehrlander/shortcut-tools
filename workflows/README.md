...
| `stage-input` | Sends what was shared to the Web Tools stage in Safari. **A Safari page sends its address, not its text**: the first device run (2026-09-24) went down the clipboard route, which fits a page's text coercing to its article rather than its URL, so the item's type is read first (`Safari web page`, the string the corpus compares against). A payload of 1 to 4,000 characters rides the link as `#take=`; anything else (an image, a file, long text) goes to the clipboard and the app opens on the stage for one tap of Paste. With no input it stages the clipboard. It logs `type` and `chars` before choosing, so a run reports its own route; the log sits ahead of the clipboard write, since `Log-Repo` writes the clipboard too. 27 actions, in the share sheet. |
