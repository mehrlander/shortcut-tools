...
| `run-pick` | `Run-Choice` in four cards: the input is the verb list, one name per line, and the payload is the clipboard. It drops `Get-Shortcut` entirely, because Run Shortcut is a legacy action whose target is a plain string key and the chosen name resolves on its own; `run-steps` already carries a bare token string there with no `WFWorkflow` dict. Gives up the self-demo prologue and an arbitrary payload. |
