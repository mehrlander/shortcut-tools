...
| `back-entry` | **The shortcut Back Tap is bound to.** One card, calling `run-backtap` with Shortcut Input. Bind Back Tap to this once and the dispatcher becomes an ordinary re-install: a save-over import does not preserve the system's reference, so re-installing the *bound* shortcut costs a scroll through the whole library in Settings to re-select it, and this is never the bound shortcut being re-installed. Named `Run-Gesture` for one afternoon on 2026-09-14, which called Back Tap a gesture and said nothing about which one; any copy still on a device under that name is an orphan and can be deleted. The cost it carries is this branch's own Fault 2 moved to the root: its only action names its target by string with no fallback, so `Run-BackTap` going missing or being renamed kills the gesture silently. `catalog.json` is where that name is visible to an audit. |
