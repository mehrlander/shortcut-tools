...
| `stage-input` | Sends what was shared to the Web Tools stage in Safari. **A Safari page becomes two stage items in one link**: Run JavaScript on Web Page returns them as the JSON the app's `#gz=` payload carries (`<host>-link.txt` with `location.href`, `<host>.html` with the document's outerHTML), and `Show-HtmlViaZip`'s proven sequence gzips it (name it, text, Make Archive `gz`, Base64 with no line breaks). Standard base64 rides as is: the app's decoder maps only `-` and `_` and pads, held by web-tools' `stage.test.mjs`. The page is recognised by its type, `Safari Web Page`, the string the phone reported on 2026-09-24. How large a link Open URL and Safari will carry is **unmeasured**, so the log row records the payload's length (`gz`). Anything else: text of 1 to 4,000 characters rides `#take=`, and the rest (an image, a file, long text) goes to the clipboard with the app opened on the stage. With no input it stages the clipboard. 33 actions, in the share sheet. |
