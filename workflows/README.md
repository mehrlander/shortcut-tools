...
| `probe-webview-caps` | The remaining sheet questions in one run: origin, `DecompressionStream`, SpeechRecognition, localStorage, a jsDelivr script tag, and the microphone. It copies its own result line to the clipboard, so the answer is a paste rather than a transcription. |
