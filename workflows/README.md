...
| `run-pick` | `Run-Choice`'s contract in five cards, without `Get-Shortcut`. Item 1 is the payload, item 2 the verb names one per line; both arrive as parameters, so it composes and `Run-Steps` can pipe into it. The resolver goes because Run Shortcut's target is a plain string key, which `run-steps` already relies on. Still no self-demo prologue, so a bare run has nothing to build the list. |
| `pick-clip` | The link adapter for `run-pick`, and the only place the two-argument awkwardness lives. A `shortcuts://` link carries one string, so it cannot hand over a two-item list. The payload may contain anything including newlines, so it cannot be a delimited field; the verb list is bounded, so it can. Takes the bounded one as input, reads the payload from the clipboard, pairs them. |
...
| `bench-call` | One bench op run against the clipboard, inside a shared Safari page. The worked verb for the coprocessor route: the page holds the libraries, so the script sent is a single `completion(bench.call(op, base64))` and the parse cliff never applies. Payload base64 because it is interpolated into a string literal. |
| `bench` | The coprocessor with nothing hosted and nothing open. The op is Shortcut Input, one bare word, so a link bakes it in; the payload is the clipboard. It builds `pages/bench-run.html`, encodes it into a `data:` URL, coerces that to rich text and reads the answer back, fetching one library from jsDelivr only when the op needs it. Measured end to end by `probe-coercion` on 2026-08-28: 40,214 bytes fetched, evaluated and used in 738 ms. |
| `probe-bench` | `Bench` end to end on a payload it carries itself. It sets the clipboard before calling, rather than asking for something to be copied, and logs the result rather than leaving it on the clipboard to be pasted back. `Run-Steps` cannot stand in: it splits its input into shortcut names, so a payload has no slot. |
...
| `probe-inline-bench` | Whether a `data:` URL page can pull a library off the CDN and use it before the rich-text coercion reads it. The route left after `probe-tab-js` closed the tab entry: it needs no live page, so no share sheet. Everything in the page is synchronous, since an async resolution is lost with no error. Four lines back in one tap: the fetch, the eval, the library's output, the elapsed cost. |
| `probe-coercion` | Which of five inputs the rich-text coercion actually renders, logging each before the next runs. `probe-inline-bench` came back as one empty line and could not say whether the coercion refused a `data:` URL, ran no script, or timed out on the network; these four separate those. Static HTML, a script that rewrites its own line, a real `https` URL, then the sync/async network pair. Read the first empty line: everything above it worked. |
