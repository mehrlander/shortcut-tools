...
| `describe-input` | The same job as `get-file-info` with nothing to call out to: the item loop, the JSON array, the function, and the data-URL evaluator all inline. 29 actions against 10 plus four dependencies totalling about 60, and no name to go stale. Takes a bare array, so the count test moved into the function. |
