...
| `choose-claude` | The Claude-app menu. Its first line opens the copied branch's session in Web Tools by running `claude-session`; the others dispatch by name through the same arm `choose-backtap` uses, so a further option is one line of the Get Text card. 11 actions. Until 2026-09-03 that line read "Read the session aloud" and built `session.html#branch=` around the clipboard in a URL card, which arrived empty on the phone. Both halves were wrong: the label named a thing the page does rather than what the tap does, and the branch does not need to travel in an address at all. |
| `claude-session` | The session picker, and the fullest use of the `gh-recent-branches-picker` route: JavaScript computes the menu on the phone, Shortcuts draws it, and Web Tools displays the result. 21 actions. The clipboard arrives as base64 and the token through `Inject-🎟️GitHubToken`, both by substitution into [`pages/session-lines.html`](../pages/session-lines.html); the page becomes a `data:` URL that is coerced to rich text rather than opened, which runs it and hands back its text. That text is a caption and its rows, so the menu's prompt says what was found (`1 on claude/…`, or that the clipboard held no branch) and each row is a session. Choosing one opens `session.html#id=` in Safari, where the record is read and the page's own listen button is. A failed read never reaches the menu: the caption carries `ERROR`, the chain shows it and logs it with its build id. |
