...
| `gh-recent-branches-picker` | The same page read back as text and fed to Choose from List. The fallback that needs no `Show-Html`. Its Replace Text input was an inferred shape until 2026-09-03, when a census of 754 uses found the corpus unanimous for the token-string form; `test/parameter-shapes.test.js` holds it now. |
...
| `dump-folder-zip` | One folder's shortcuts, zipped whole and **committed**. The four cards the three-dumps note has always recommended, `Get My Shortcuts` with a Folder, `Get Dictionary from Input` to key them by name, `Make Archive`, `Set Clipboard`, with `dump-recent-zip`'s tail behind them. It declared no `name` until 2026-09-14 and so could only be pasted; it can be installed now because it answers a question nothing else here can. **The manifest carries no `folder` column**, deliberately, since a shortcut with no folder makes that column misalign, so nothing in this repo could otherwise say what is in a folder. Zipping one and committing it is the only reader of folder membership there is, which makes it the check on `library-move` as well as a dump. The Folder is `Ask`, as it is in all 15 corpus instances of that parameter: one tap at run time, and the same tap is the size control. **Ends on the log page** (2026-09-25): on a confirmed write it logs `{"op":"dump","file":…}` through `log-repo` and opens `shortcut-log.html` in a sheet, where the new row sorts to the top with the page's five-minute ring. Only after confirmation, so a refused write keeps its clipboard fallback and its Show Result. `sync-manifest` does not follow them: it runs unattended at 02:00, and a sheet then lands on a locked phone. |
...
| `dump-recent-zip` | `dump-recent`'s selection with `dump-folder-zip`'s packaging: the same days parameter and the same Last Modified filter, then **`Get Dictionary from Input` and `Make Archive` instead of a Repeat**. 22 actions against 25, and the count is not the point: the loop is gone. `dump-recent` pairs `Get Name` with `Get File of Type` once per matching shortcut, and keying the set by name removes the only reason that pairing existed, which the three-dumps note above has argued since it was written. Spliced rather than re-authored: actions 0 to 9 and the authenticated PUT are `dump-recent`'s own cards, and the two replacing its loop have 192 and 33 corpus instances between them. It commits a `.zip` rather than a `.txt`, which is the format `index-dump.py` already reads, and it ends through `check-api-reply` and names what landed rather than asserting `Dumped.` over whatever came back. **Ends on the log page** (2026-09-25): on a confirmed write it logs `{"op":"dump","file":…}` through `log-repo` and opens `shortcut-log.html` in a sheet, where the new row sorts to the top with the page's five-minute ring. Only after confirmation, so a refused write keeps its clipboard fallback and its Show Result. `sync-manifest` does not follow them: it runs unattended at 02:00, and a sheet then lands on a locked phone. |
...
| `dump-named` | Exports only the shortcuts named in its input and commits them back. The precise form, for when a manifest has already said which. **Ends on the log page** (2026-09-25): on a confirmed write it logs `{"op":"dump","file":…}` through `log-repo` and opens `shortcut-log.html` in a sheet, where the new row sorts to the top with the page's five-minute ring. Only after confirmation, so a refused write keeps its clipboard fallback and its Show Result. `sync-manifest` does not follow them: it runs unattended at 02:00, and a sheet then lands on a locked phone. |
...
| `choose-runlink` | A copied `shortcuts://run-shortcut` link in, the run offered, **or nothing**. 24 actions. It reads the target's name out of `name=` and **checks that name against the device's own library** with the two cards `run-list` resolves a name with, `Get My Shortcuts` and a `Name is` filter, asked as a question instead of as a call. The answer is the picker's prompt, so a link naming something uninstalled says so before the tap rather than failing silently after it. The link itself is handed to Open URL, which is how `Fav` has run a `shortcuts://` link from inside a running shortcut since before this repo existed, and which leaves `name`, `input` and `text` to the same parser a tap uses. Declining is the row `Out`, spelled the way `choose-backtap` spells it, because the clipboard is sticky: a link copied once would otherwise shadow every other route until something else was copied. **Its callers recognise the link themselves and only then call it**, which is the opposite of `get-app-route`'s arrangement and deliberate: that receiver was already installed and proven when a dispatcher started calling it unconditionally, and this one was not. A `Run Shortcut` card at the top of the bound gesture makes every tap depend on a second install existing under exactly the right name, and when it does not the gesture dies before reading anything. Measured 2026-09-14, on the first tap after the first install. Given no input it reads the clipboard, and it matches the link itself, so it still works standalone and the caller's guard is not load-bearing twice. The prompt's emoji sit **after** every anchor, since a code-point offset and a UTF-16 offset disagree below an astral glyph: the anchor gate caught that here before it reached a device, which is the `🎟️` hazard `run-op` documents arriving through a picker title. |
...
| `double-backtap` | **What Back Tap double is bound to**, and all it says is which gesture fired: the text `double-back`, handed to `route-gesture`. Two actions, and it never changes, so the dispatcher behind it is never the bound shortcut and a re-install never costs a Settings visit. It declares `WFWorkflowTypeShowInSearch` and nothing else: a shortcut has to be listed somewhere to appear in the Settings picker, and the no-input key that went missing for a fortnight cannot be missing from a file that reads no input. Named `Gesture-BackTap2` for one day, which is the suffix Apple appends to a declined save-over and which 58 of the library's 74 sediment-tier shortcuts carry; `Back-Double` was rejected for sitting beside the dead `Back-DoubleTap` and `Back-TripleTap` copies in the Settings list. |
| `triple-backtap` | **What Back Tap triple is bound to**: the text `triple-back`, handed to `route-gesture`. The double stub's twin, and like it never changes. |
| `route-gesture` | **One label in, that label's shortcut run on the clipboard.** The table is three lines now: `[double-back]=Run-BackTap`, `[triple-back]=Choose-Manage`, and `[web-token]=Copy-GitHubToken`, read back by the same lookbehind match `get-app-route` uses, so adding a caller is one line here plus whatever it calls. An unrecognised label falls back to `Run-BackTap` rather than doing nothing, because silence is the worst outcome for a bound gesture, and that fallback is also what lets a web page name a label the device has not learned yet: the tap raises the back-tap menu instead of failing, which is a visible answer to whether the tap arrived at all. It takes the clipboard with an action and passes it on, rather than relying on the file-level key that supplied it invisibly until 2026-09-14. **The row said `[back2]` until 2026-09-19** and the table never held that label; the file has read `double-back` since `double-backtap` was written to emit it. |
| `copy-github-token` | **The stored token onto the clipboard**, for a browser that has lost its own copy. Three actions: the bare `🎟️GitHubToken` sentinel through `Inject-🎟️GitHubToken`, then Copy to Clipboard. Reached as `[web-token]` through `route-gesture`, which is how Web Tools' floating button asks for it: a home-screen web app on iOS keeps its own storage, separate from Safari's, so the token saved in one browser is not the token the other reads. **It does not log, and that is the one place this repo's report-itself rule inverts.** `Log-Repo` opens by setting the clipboard to its own input, so a log call in this chain leaves a window where the clipboard holds JSON rather than the token, and the reader who switches back inside that window pastes the wrong thing. The payload here IS the clipboard, so the return channel would destroy what it was reporting on. The confirmation belongs on the far side instead: the page checks the shape of what it was given and says so. **No `🎟️` in the name**, unlike the three shortcuts that hold or set the token, and deliberately: this one is reached by a name the router computes at run time rather than by a literal, and an astral glyph in a computed name is the one link in that path the corpus does not attest.
| `run-backtap` | **The shortcut Back Tap actually runs** (confirmed from the Settings page, 2026-09-14). `Back-DoubleTap` with its five app branches replaced by one `get-app-route` call, a **guarded** `choose-runlink` call above it, and a Stop and Output the dictation arm was missing: 37 actions against 67. Spliced from a device dump rather than re-authored, so every card kept is the card running today. **The splice dropped the file-level settings and they are back**: the conversion carried the actions and not the `workflow` block, so the published plist declared no `WFWorkflowNoInputBehavior` and every back tap reached it with Shortcut Input empty. That went unnoticed for a fortnight because the app routes do not depend on it: `choose-claude` opens with a `Get Clipboard` **action** and `choose-backtap` reads the `Clipboard` variable, so Claude, Audible, Music, Roku and Shortcuts all kept working while every unmapped app fell to the no-input arm. Nothing on the clipboard reaches those keys, which is the reason `plist.py` exists. |
| `choose-manage` | The triple-tap target: every shortcut whose name contains `Manage`, as a menu, and the pick runs. Versioned from the 2026-08-13 dump, where it already sat; the device copy is unchanged. |
...
| `choose-claude` | The Claude-app menu, 17 actions and no rows of its own. It reads the clipboard, hands `session-menu` and that text to `run-op`, and draws what comes back: `caption` as the prompt, `menu` as the rows. A chosen row is a key into the `urls` map the same call returned, so a hit opens a page and a miss runs the row as a shortcut name, which is the same has-value idiom `get-app-route` uses and what lets one list mix sessions with verbs. `Out` is tested explicitly rather than left to fail a name lookup, and since 2026-09-14 **an empty choice is tested before it**. The op guarantees `menu` is never empty, an ERROR result included, and that guarantee holds: the op was run here against seven clipboard shapes, catch path included, and returned a menu every time. **`run-op`'s own non-200 reply is not the op**, and it predated `menu`: it carried `caption`, `rows`, `urls` and `error` only. So one 401 gave an empty list, Choose from List returned nothing, an empty Choice is "not Out", and the chain walked into the variable-keyed `Get Dictionary Value` below and stopped with "No key was provided for dictionary action" three shortcuts from the cause. Reported from the device 2026-09-14; `run-op` now carries `menu` on that path and `test/parameter-shapes.test.js` holds its error reply to whatever keys this shell reads. The new arm names it `menu-empty`, logs it with the build id and shows what failed, the same shape as the `menu-miss` arm beside it, so the next occurrence arrives in `shortcuts/log/` instead of as a card error. `claude-session` carries the identical hole at its own action 12 and is not fixed here, being off the back-tap route. **So the menu lives in web-tools now**, in `lib/ops/session-menu.js`: its wording, its order, its verbs and the address each row opens are a commit there and cost no install here, which is the rule at the top of `CLAUDE.md` applied to the menu itself. The header says whether the clipboard held a branch this estate recognises, and the rows change with the answer: a recognised branch leads with its session, an unrecognised one leads with a row that opens `session.html#branch=`, the route that walks the store and reaches no cache. `test/parameter-shapes.test.js` holds the shell's shape; web-tools' `tools/test/ops.test.mjs` holds what the op must always return, including the verbs, so a failed op still draws a working menu. **Wrong 2026-09-08 → the row above:** it described a four-row menu whose first row, **Session on this branch**, was offered whether or not a branch was on the clipboard, and whose prompt was the literal string `Claude`. Both are gone. |
| `choose-gpt` | The ChatGPT-app menu, 9 actions and **no network at all**: a Text block of rows, one Dictionary card of destinations, a Choose from List, and Open URL on whatever the key returns. `Out` is absent from the dictionary rather than tested, so it falls through the has-value arm and ends the chain; no card is wasted on it. It reads no Shortcut Input, so the install needs no toggle. **It was 14 actions and carried `choose-claude`'s open-or-run dispatch** (a value containing `://` opened, anything else run as a shortcut name), for one row that ran `claude-session` against the twelve newest sessions. The owner cut that row on 2026-09-19, leaving the run arm reachable by nothing, so the arm went with it and the `://` test collapsed back to a plain has-value guard. Restoring it is a small edit and one install, which is the reason not to keep it warm against a row that may never exist. **Two destinations are the owner's own, not this repo's guess**: `Nav-CurrentApp` has keyed a dictionary on `ChatGPT` and `Claude` since the 2026-08-13 dump, which is where the display-name spelling `get-app-route` now routes on comes from, and where `https://chatgpt.com/` is attested. **`https://chatgpt.com/codex` is not attested anywhere**: it appears in none of the fifteen dumps, whose complete AI-destination inventory is twelve URLs, and every probe of the host from a sandbox returned HTTP 403, so it is unverified until a tap says otherwise. A wrong path lands on the ChatGPT home rather than breaking the menu. **Voice mode was cut from the first build and is worth a second look**: `com.openai.chat.OpenVoiceModeIntent` is attested in the corpus (`Voice Chat`, bundle `com.openai.chat`, team `2DC432GLL2`) and is the one ChatGPT capability no URL reaches, but all seventeen corpus uses of that vendor's intents were authored on device and nothing here has ever generated a third-party App Intent card into a signed plist. That is a shape to debut in a second install, not in the one that has to work. |
| `run-op` | Name an op in web-tools, hand it one line of input, get its JSON back. 7 actions, and the layer `docs/idioms.md` predicts: `Get-FromJs` is the one door to JavaScript, and this adds addressing to it. The op's file is fetched **through GitHub's contents API** by name, inside the expression `Get-JsonFromJs` evaluates, so the code lives in the repo and runs on the phone. Three things about that fetch are load-bearing. The `Accept: application/vnd.github.raw` header is what makes the response the file rather than a JSON envelope around a base64 body. The `Authorization` header buys the authenticated rate limit and would reach a private repo, and it is the same pair the op sets on its own data fetch, which is why this route is already known to survive the `data:` page's null origin. And **no purge follows a merge**, because `private, max-age=60` leaves no shared cache to go stale; the timestamp on the address covers the phone's own sixty seconds. Its non-200 arm returns a dictionary it builds itself, and that object has to carry every key the caller reads off a success: it did not carry `menu`, which is the whole of the 2026-09-14 failure, and a generic runner spelling one op's schema is the standing smell behind it. The input rides in as base64 so nothing it contains can break the expression, and the token reaches the op as an argument through `Inject-🎟️GitHubToken`, which Run-Op calls itself: `Get-JsonFromJs` calls it only inside its no-input demo, a reading error that cost one device run (2026-09-03, `TypeError at headers`). Both U+FFFC anchors sit above the token placeholder, since `🎟️` is two code points and three UTF-16 units and an anchor below it lands differently depending on which convention reads the file. Every future chain that needs computation is this one action plus what it does with the result. The contract on the other side: web-tools `lib/ops/README.md`. |
| `claude-session` | The session picker as its own shortcut, 15 actions, off the back-tap route since `choose-claude` began drawing the same op's menu inline. Kept because it is installed, still works, and reads only `caption` and `rows`, which the op still returns unchanged: `menu` is the key that carries the verbs, and `rows` is the sessions alone. A caption carrying `ERROR` is logged with the build id and shown instead of opening a menu, which is the half of it `choose-claude` does not reproduce. The logic is web-tools' `lib/ops/session-menu.js`, which the app can run too. **It shares one shape with the bug that took `choose-claude` down on 2026-09-08**: its Choose from List is fed a value out of the op's JSON, so the chosen item is not natively a string. It only ever uses that item as a dictionary key, which the corpus does do bare (4 instances), rather than in a text condition, which the corpus never does and which is what actually failed. So it may well work; it has never been observed to. Fixing it is one Get Text card and an install, and is worth doing the next time anything else about it changes rather than on its own. |
...
| `check-github-token` | Is the stored token live, and can it reach the private store. Three actions: the page, `Inject-🎟️GitHubToken`, `Show-Html`. It was device-only until 2026-09-14 and is here now because it needed to grow buttons. **Two fetches, not one**, and the second is the point: `api.github.com/user` says the credential parses, and `repos/mehrlander/web-tools-private` says it can do the job, since a token with no `repo` scope authenticates cleanly and still cannot write a log row. So `limited` is its own state beside `stale`. When the state is anything but `active` it offers the two links that fix it, a pre-scoped GitHub token form and `set-github-token`, which is the whole reason it stopped being a badge. |
| `set-github-token` | The clipboard becomes the stored token. Eight actions on `set-shelf`'s write path with the key changed: read `Snippets/Managed/config.json`, set `🎟️GitHubToken`, rename, save with overwrite. It reaches the file the way `self-name` does, a bare `fileLocation` with no `crossDeviceItemID` and no `fileProviderDomainID`, so no iCloud account identifier rides into a public repo. Given no input it takes the clipboard, so the link needs no payload. It ends by running `check-github-token` rather than announcing success, which is the diagnostic-returns-itself rule applied to a write: the badge that said `stale` says `active`, or it does not. |
| `find-shortcut` | Every shortcut whose name **contains** this one, and the copy you pick opened in the editor. Eleven actions. `contains` rather than `is` is the whole design: a duplicate is named `Foo 1`, so an exact match is the one test that cannot see the thing this exists to find. One copy is the ordinary case and it says nothing; more than one draws the picker, which is `count [2] 1`, the corpus's own idiom for "choosing needs more than one" (seven shortcuts). **Wrong 2026-09-14 → here: "the Shortcuts app offers no search action".** It offers `com.apple.shortcuts.SearchShortcutsAction`, spelled `searchinshortcuts` in this repo's own `actions.json`, taking a query and a Folder. The corpus search that missed it was right and the conclusion drawn from it was not: none of the 606 shortcuts uses that action, and the dictionary was never asked. Rule 1 spells out the licence a silent search grants, "I did not find one", never "there is none", and this row asserted the second. What the two candidates actually do still holds: `OpenNavigationDestinationAction` reaches folders and `Share Sheet` and takes no query, and `OpenWorkflowAction` resolves one shortcut and so returns the original rather than its duplicate. Whether `SearchShortcutsAction` returns matches or opens the app at a search is not settled here, and its parameter names are not in the dictionary, so this chain keeps `Get My Shortcuts` plus a `Name contains` filter until a device dump supplies the card. **It opens rather than deletes**, because deletion is the fourth step here and never the first, and the editor is where a person can see what they are about to lose. Returns the count, so a caller can branch on it. |
...
| `check-api-reply` | A GitHub reply in, the same reply out, and a notification if it refused. Six actions, and the one place every token-dependent write now goes to be judged. GitHub answers a `message` key only when it refused, on both the contents endpoints this repo uses, so one `Get Dictionary Value` decides it and no caller has to know an HTTP status. **The channel is a local notification on purpose**: the failure it exists to report is a dead credential, and every other channel here, `Log-Repo`, `Sync-Manifest`, `Run-Op`, the log page, needs that credential to be alive. A probe cannot return itself when the return path is what failed. Between 2026-09-10 and 2026-09-14 an expired token stopped three writers at once and the only symptom was four days of silence, which is the case this exists to make loud. It passes the reply through, so it sits inline in a chain rather than at the end of one. |
| `log-repo` | The return channel. Writes the input to the clipboard first and unconditionally, then commits it to `shortcuts/log/` in web-tools-private, so a diagnostic ends in a commit rather than in a question. **Silent on success since 2026-09-25**: it runs in the middle of other shortcuts (`stage-input`, `library-paste`), so the Show Result it used to end on interrupted them and said nothing a person could use. A refused write still reaches the screen, as a notification from `check-api-reply`. A shortcut that wants to report ends on the log page itself, as the dumpers do. |
| `capture-link` | The share sheet's end of that channel: whatever was shared, straight into the repo log. Two actions. |
| `stage-input` | Sends what was shared to the Web Tools stage in Safari. **A Safari page becomes two stage items in one link, and the page does the work**: one Run JavaScript on Web Page card builds the items (`<host>-link.txt` with `location.href`, `<host>.html` with the document's outerHTML), gzips them with the page's own `CompressionStream`, and returns one base64 string. **Safari's compressor refused a Blob stream's chunks** on the first device run (2026-09-25, `TypeError: Invalid type should be ArrayBuffer`), so the script writes an ArrayBuffer to the writer, falls back to a Uint8Array view, and reads the output with a reader loop rather than `Response`; an `ERROR <step>: …` result names the step that failed, which rides `#gz=` as is. Moved into the page on 2026-09-25 after two device runs of the previous build logged nothing: that build returned the page and gzipped it in Shortcuts (name it, text, Make Archive, Base64, `show-html-via-zip`'s sequence), and the run before it had returned 209,187 characters from the same kind of script without trouble, so the stall sat in the Shortcuts steps. Returning one plain string from one expression is also `Get-Nice`'s shape, the owner's fast route to a page's HTML. In Chromium the script takes 37 ms on a 184 KB page and the app's decoder reads its output. The page is recognised by its type, `Safari Web Page`, which `Get-Nice`'s own condition also uses (its August dump spelled it `Safari web page`). The log row records the payload length (`gz`); a very short one means the script returned `ERROR …`. Anything else: text of 1 to 4,000 characters rides `#take=`, and the rest goes to the clipboard with the app opened on the stage. With no input it stages the clipboard. 29 actions, in the share sheet. |
| `stage-input-card` | The one card that gives `Show-Loop` a Stage case: `Run Stage-Input` on the shortcut's input, then Stop and Output so Show-Loop's `repeat` does not bring the menu back, pasted under a new menu item. |
| `stop-output-card` | Stop and Output alone, for a `Show-Loop` whose Stage case already has its `Run Stage-Input` card: pasted under it, so the menu does not come back after Safari opens. |
...
| `library-paste` | **The install that needs no signing**, and since 2026-09-25 **it says first whether the install is needed**. The link carries three lines: name, payload address, and the build id (`pack.py --install` adds it). Before changing anything it reads the installed copy as JSON (`get-shortcut-json`'s route: My Shortcuts, a Name filter, as file `public.json`) and looks for the build id, then shows one of three standard alerts, each with Cancel: *Already installed* (that build is already there; OK installs it again), *Replacing* (an older copy is there; it is deleted after OK, and after the log row, so replacing `Log-Repo` itself does not delete the logger before it runs), or *Installing* (the name is new). Then it logs, puts the packed actions on the clipboard, creates the empty shortcut and opens it, and a person pastes. The build is found only in a chain that stamps `#BUILD#`; `pack.py --install` says when one does not. **It logs after the go-ahead and before the copy**, because `Log-Repo` writes the clipboard, and a log after the open never runs (`ui-action-breaks-what-follows`), which is why `library-install`, whose log sits after its open, has no install row in `shortcuts/log/`. The row carries `target`, the build asked for. No paste reaches the file's own settings, so `pack.py --install` prints the toggles (Show in Share Sheet and the like) to set by hand. 23 actions. |
...
| `library-move` | Each named shortcut into a folder created by name, and a log row saying how many names actually resolved. 13 actions, five of them `library-stage`'s own: split the names, repeat, resolve each by an exact `Name` filter, move. **Wrong 2026-09-14 → here:** the first version shipped the move card's folder slot unset, on the reasoning that a folder is addressed by a device-local UUID and could be filled in once in the editor the way `library-stage`'s `Stage` was. It was installed, run over 51 names, and moved nothing: the dumped copy came back with **no `folder` key at all**, so an omitted parameter is not a blank waiting to be filled. `CreateFolderAction` takes a plain text name and its result now feeds the slot, which is the one unproven shape here (one corpus instance, output unused) and is the thing a run settles. The log row reports `asked` and **`found`**, the second being what the `Name` filter resolved inside the loop; the first version reported only the input count, so 51 names matching nothing read as a clean 51. That is the fourth chain this week to assert a success it never checked, after `Synced.`, an empty `Confirmed:` and `Dumped.`. **Measured 2026-09-14: 23 of 51 moved**, and the folder zip showed them to be the first 23 names in order with the 24th the first name not installed. A `Name is` filter that resolves nothing hands `Move` an empty list and the Repeat ends there, silently, because `Log-Repo` is the last action and a dead loop never reaches it. The body is wrapped in a has-value test now, so a miss is skipped. That run also proved `CreateFolderAction`'s output satisfies a folder slot, and the next day proved the create **refuses a folder that already exists**, which makes create-then-move single-use: right once, then dead at action 0. Both measurements together leave one shape: the card carries a real folder entity, shipped here as `Utilities` because a forgotten placeholder should be untidy rather than dangerous, and a person re-points it once in the editor. An omitted key renders as no field; a present one is a field. Dump the chain afterwards and the chosen folder's UUID can be baked in for good. It differs from `library-stage` in intent rather than mechanism: staging nominates for deletion, this collects a working set. |
...
| `probe-apps` | The probe that settled the app question, kept as its record. Eleven cards around `is.workflow.actions.filter.apps`, **Find Apps**, which the ToolKit catalog lists on iOS and the device refuses: *"This action can only run on Mac."* Imported clean and wired correctly, so what failed was the catalog inference, not the chain. It declares no `name` and so has no plist: there is no receiver to install, because there is nothing the phone would run. Restore the `name` to get one on a Mac. |
| `read-shots` | Screenshots in, text in the repo, one tap. Select photos, OCR each, combine, commit through `Log-Repo`. The capture route for anything the phone will only show and never hand over: an app list, a picker, a settings pane. It uses `WFImage` on `extracttextfromimage`, which the corpus runs 7 times on device while the ToolKit catalog calls the action macOS-only and names the key `imageFile`. The corpus wins that disagreement. |
| `log-pageshot` | A URL in, a screenshot of it in Safari committed to `shortcuts/shots/`, one tap. The visual half of the return channel: `read-shots` OCRs screenshots the reader picked by hand, and this takes its own. 27 actions, all native, because idiom 4's escape hatch does not reach a screenshot, an image resize, or a PUT carrying an injected token. **The Show Web View sheet is gone from it and cannot come back.** Its first version raised the page in the sheet, waited, and shot; `probe-sheet-blocks` then measured a 33-second gap across the sheet, so that leg could never have worked, and no alert or notification can gate a shutter from behind a sheet either. Safari is the only route to a live page. Two shots, at 5 and 12 seconds, arriving side by side as one image: a single instant is a bet on how fast the page loads, and the 2026-09-09 run lost that bet twice. The alert says to leave the phone alone and the notification says when it is safe to pick it up again, which is the part a fixed delay cannot supply on its own. |
| `probe-sheet-blocks` | Does a shortcut keep running while the Show Web View sheet is up, or resume only when it is dismissed? Ten actions: read the clock, raise the sheet, read the clock again, log both times in one row. A delta near zero means the run continued behind the sheet; a delta matching how long the sheet was left up means it blocked. Nothing to answer, per the rule that a diagnostic returns itself. It exists because `docs/shortcuts-format-notes.md` asserts the run continues and the corpus leans the other way (11 of 17 chains using `showwebpage` put it last in a branch), the two `Probe-WebView` chains both end at the sheet and so tested nothing about it, and the 2026-09-09 `log-pageshot` run could not separate the two readings. Every screenshot design branches on the answer: if the sheet blocks, no single shortcut can photograph its own sheet, and Safari is the only route to a live page. |
| `log-pagerender` | The same job as `log-pageshot` with the screenshot taken out. Get Contents of Web Page loads the address, Make PDF renders it, the first PDF page becomes a JPEG, and that is committed. Nothing is displayed and no shutter fires, so there is no delay to race and nothing on screen can end up in the capture, which is what the 2026-09-09 run of `log-pageshot` produced: both its legs caught the app switcher, because a fixed delay is a bet on where the reader will be five seconds from now. The open question is whether Make PDF runs the page's own JavaScript, and the committed image answers it without a question being asked, since a page whose script never ran arrives showing its pre-script state. `pages` in the log row is the PDF's page count, so a first-page-only capture reports how much of the page it left behind. Shapes from the corpus's only PDF-to-image chain, Gemini via API: split, take the first page, `public.jpeg` at 150. |
...
| `probe-unattended` | Does the phone do its bookkeeping while nobody is holding it? Four legs, each committing before the next and ordered by risk: a control row PUT straight to `shortcuts/log/` (token injection, no UI), `Get-FromJs` on `(()=>1+1)()` written the same way, `Sync-Manifest`, then `Log-Repo`. The last two each end in a Show Result, which is the thing a background run may not survive, so they go last. Run it once by tap for the attended baseline, then from a time-of-day automation with Run Immediately. Read the first missing row: everything above it works unattended. Built for the unattended-venue proposal, home PR #585. |
| `run-probe-unattended` | One `Run Shortcut` card naming `Probe-Unattended`, for pasting into a blank time-of-day automation. The automation editor is the ordinary action editor, so the card lands there like anywhere else, and pasting it skips searching the action list and then the shortcut list. Not a receiver, so it declares no `name` and stays out of `plists/`. |
...
<!-- doc-index -->
**25 sections.** Generated by `python3 tools/doc-index.py --publish`.

- [`show-html-js`: the same job, mostly in JavaScript](#show-html-js-the-same-job-mostly-in-javascript)
- [The two mechanisms the device library lacks](#the-two-mechanisms-the-device-library-lacks)
- [Payloads live in `pages/`, not pasted into the chain](#payloads-live-in-pages-not-pasted-into-the-chain)
- [Parameter shapes are load-bearing and only partly confirmed](#parameter-shapes-are-load-bearing-and-only-partly-confirmed)
- [Both probes ran, 2026-08-12](#both-probes-ran-2026-08-12)
- [The two probes, and why they were worth a tap](#the-two-probes-and-why-they-were-worth-a-tap)
- [Two walkers, and why one asks before it runs](#two-walkers-and-why-one-asks-before-it-runs)
  - [`probe-step` ran, 2026-08-23](#probe-step-ran-2026-08-23)
- [Three dumps, and which to use](#three-dumps-and-which-to-use)
- [Why `dump-selected` exists beside `dump-shortcuts`](#why-dump-selected-exists-beside-dump-shortcuts)
- [Signing here, so the device never does](#signing-here-so-the-device-never-does)
- [Installing one, rather than pasting it](#installing-one-rather-than-pasting-it)
- [Keeping the corpus current without re-dumping it](#keeping-the-corpus-current-without-re-dumping-it)
  - [Get returns, Dump delivers](#get-returns-dump-delivers)
  - [The app is a key, not a branch](#the-app-is-a-key-not-a-branch)
  - [Converting the dispatcher by splicing, not re-authoring](#converting-the-dispatcher-by-splicing-not-re-authoring)
  - [The caption path was five shortcuts and is now one](#the-caption-path-was-five-shortcuts-and-is-now-one)
  - [Why the lookup and the default are separate shortcuts](#why-the-lookup-and-the-default-are-separate-shortcuts)
  - [A default in three actions](#a-default-in-three-actions)
  - [One wire format, because the others are derivable](#one-wire-format-because-the-others-are-derivable)
  - [The precise form: manifest, then named](#the-precise-form-manifest-then-named)
  - [What the first device run changed](#what-the-first-device-run-changed)
  - [Launching a shortcut updates its modification date; being called does not](#launching-a-shortcut-updates-its-modification-date-being-called-does-not)
  - [The dumper could not dump itself](#the-dumper-could-not-dump-itself)
- [Reading a dump back](#reading-a-dump-back)
<!-- /doc-index -->
...
**The merge is the whole publish**, and no purge follows it. `Run-Op` fetches an
op through GitHub's contents API, which serves `private, max-age=60`: no shared
cache exists to hold a replaced op, and `?_=` covers the phone's own sixty
seconds. **Wrong until 2026-09-08 → this paragraph:** the fetch went through
jsDelivr, whose `s-maxage=43200` kept a replaced op alive at the edge for twelve
hours, so every publish owed a purge of the ref path. The op's own data fetch
had used the API all along and had never needed one, which is what settled it.
