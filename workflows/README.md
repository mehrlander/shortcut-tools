...
| `choose-claude` | The Claude-app menu, and the only one of these that ends in a URL rather than a name. Reading the session aloud builds `session.html#branch=` around whatever is on the clipboard and opens it in Safari, since the page reads a private store through the token Safari holds and an in-app browser may not have one. 13 actions: the menu, then two arms, a URL and the `choose-backtap` name dispatch, so a further option is one line of the Get Text card. |
