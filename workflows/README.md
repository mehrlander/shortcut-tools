...
| `stage-input` | Sends what was shared to the Web Tools stage in Safari. **A Safari page sends its address and its raw HTML** (a placeholder, 2026-09-24): Run JavaScript on Web Page returns `location.href` and `document.documentElement.outerHTML`; the address rides the link as `#take=` and the HTML goes on the clipboard, so one tap of Paste in the app stages it beside the address. The page is recognised by its type, `Safari Web Page`, the string the phone reported on 2026-09-24; the corpus's one comparison spells it `Safari web page`, and a text coercion of a shared page logged an empty string twice in `capture-link`, which is why neither is relied on. Anything else: text of 1 to 4,000 characters rides the link, and the rest (an image, a file, long text) goes to the clipboard with the app opened on the stage. With no input it stages the clipboard. It logs `type`, `chars` and `html` (both lengths) before choosing, ahead of any clipboard write, since `Log-Repo` writes the clipboard too. 35 actions, in the share sheet. |
