...
| `library-move` | Each named shortcut into one folder, and a log row saying how many were asked for. Nine actions, six of them `library-stage`'s own cards: split the names, repeat, resolve each by an exact `Name` filter, move. **The folder slot ships unset, deliberately.** `MoveShortcutToFolderAction` addresses a folder by a device-local UUID (`Stage` is `E973A4AB-…`, `Utilities` is `932CAEE1-…`, both read out of the corpus), and that UUID does not exist until the folder does, so no chain here can name a new one. It is picked once in the editor and read back from a dump afterwards, which is the one-time tap `CLAUDE.md` already records against `library-stage`. `CreateFolderAction` takes a plain text name and would remove even that, but the corpus holds one instance of it with its output unused, so whether it can feed a folder slot is unproven and this chain does not guess. It differs from `library-stage` in intent rather than mechanism: staging nominates for deletion, this one collects a working set. |
