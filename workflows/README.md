...
| `run-backtap` | `Back-DoubleTap` with its five app branches replaced by one `get-app-route` call, and a `choose-runlink` call above it: 33 actions against 63. Spliced from a device dump rather than re-authored, so every card kept is the card running today. **The splice dropped the file-level settings and they are back**: the conversion carried the actions and not the `workflow` block, so the published plist declared no `WFWorkflowNoInputBehavior` and a bare back tap reached it with Shortcut Input empty, which sends every tap to the no-input arm. Nothing on the clipboard reaches those keys, which is the reason `plist.py` exists. |
...
| `back-doubletap` | The double back tap dispatcher itself, 63 actions, recovered from the 2026-08-22 device dump so it can be revised here and re-installed in one tap. Dispatches on the current app, then on the clipboard's type; an empty clipboard opens the dictation page in the sheet and everything else falls to `Show-Loop`. Pasted HTML goes to `Show-WebView` rather than `Show-Html`, so a quick look costs no Safari tab. |
