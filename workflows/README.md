...
| `library-paste` | **The install that needs no signing**: `library-install` with `library-replace`'s delete in front. It deletes any shortcut holding the name (behind a has-value test, since a Delete handed nothing is untested), puts the packed actions on the clipboard, creates the empty shortcut and opens it, and a person pastes. 13 actions. **It logs first**, for two reasons measured elsewhere: `Log-Repo` writes the clipboard, so a log after the copy would overwrite the actions, and a log after the open never runs (`ui-action-breaks-what-follows`), which is why `library-install`, whose log sits after its open, has no install row in `shortcuts/log/`. The row therefore records the attempt, and the next manifest records the result. No paste reaches the file's own settings, so `pack.py --install` prints the toggles (Show in Share Sheet and the like) to set by hand. Emit its link with `python3 tools/pack.py <chain> --install --ref <sha>`. |
