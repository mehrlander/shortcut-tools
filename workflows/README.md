...
| `copy-github-token` | **The stored token onto the clipboard**, for a browser that has lost its own copy. Five actions: a log row, then the bare `🎟️GitHubToken` sentinel through `Inject-🎟️GitHubToken`, then Copy to Clipboard. **The log call goes first and the copy last**, because `Log-Repo` opens by setting the clipboard to its own input, so the two in the other order leave the JSON where the token should be. Reached as `[web-token]` through `route-gesture`, which is how Web Tools' floating button asks for it: a home-screen web app on iOS keeps its own storage, separate from Safari's, so the token saved in one browser is not the token the other reads. **No `🎟️` in the name**, unlike the three shortcuts that hold or set the token, and deliberately: this one is reached by a name the router computes at run time rather than by a literal, and an astral glyph in a computed name is the one link in that path the corpus does not attest.
