...
| `probe-apps` | Every installed app as name and bundle id, into the repo log in one tap. Eleven cards around `is.workflow.actions.filter.apps`, the **Find Apps** action nothing in the corpus had ever used: no filter and no limit, so its source is the whole library. The catalog lists it on iOS and only the device can confirm that, which is the one thing this probe is for. The payload is a `header()` line over tab-separated rows, so a name carrying a quote cannot break the parse. |
