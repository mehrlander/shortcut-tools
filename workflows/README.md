...
| `ask-grok` | Hands whatever it is given to Grok, puts the answer on the clipboard and shows it. The prompt is not baked in: the caller composes it, so one receiver serves every button that ever wants to ask something, rather than one shortcut per question. `dictate.html`'s Ask Grok row is the first caller. |
