...
| `choose-claude` | The Claude-app menu. Reading the session aloud runs `claude-session` on the clipboard; the other lines dispatch by name through the same arm `choose-backtap` uses, so a further option is one line of the Get Text card. 11 actions. Until 2026-09-03 the first arm built `session.html#branch=` around the clipboard and opened it in Safari, and the page came up empty on the phone with the branch still on the clipboard; the receiver replaces the URL handoff rather than diagnosing it, since Show-Html delivers the clipboard by substitution and never through an address. |
| `claude-session` | The session behind the copied branch, read aloud on the phone. Two actions, the shape of `gh-recent-branches`: the page as text, then `Show-Html`, which substitutes the clipboard and the token and opens the page in Safari. The page prints what the clipboard held and what branch it reduced to, reads `state/sessions.json` from web-tools-private for the sessions on that branch and the latest dozen regardless, and on a pick reads the record and speaks it a card per ask through web-tools' `kits/read-aloud.js`, the same reduction `session.html` uses. The footer hands the current card to `Speak-Text` for the back tap's voice, and links the full page by id, so the branch never travels in a URL. Reads [`pages/claude-session.html`](../pages/claude-session.html). |
