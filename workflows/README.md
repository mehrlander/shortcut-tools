...
| `log-pageshot` | A URL in, a screenshot of it committed to `shortcuts/shots/`, one tap. The visual half of the return channel: `read-shots` OCRs screenshots the reader picked by hand, and this takes its own. 26 actions, all native, because idiom 4's escape hatch does not reach a screenshot, an image resize, or a PUT carrying an injected token. Two legs and one commit: the page is raised in the Show Web View sheet, shot, then opened in Safari and shot again, and the pair arrives side by side as one image. The second leg is insurance against the one thing the corpus cannot settle, whether the run continues behind the sheet or resumes only when the sheet is dismissed. `docs/shortcuts-format-notes.md` asserts the first; 11 of the 17 corpus chains using `showwebpage` put it last in a branch, and the ones that do not follow it with a menu or an exit, which is what resuming on dismissal would look like. Until a run says which, the combined image says it in one glance. |
