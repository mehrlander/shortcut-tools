...
| `route-gesture` | **One label in, that gesture's shortcut run on the clipboard.** `[back2]=Run-BackTap`, read back by the same lookbehind match `get-app-route` uses, so adding a trigger is one line here plus a two-action stub. An unrecognised label falls back to `Run-BackTap` rather than doing nothing, because silence is the worst outcome for a bound gesture. It takes the clipboard with an action and passes it on, rather than relying on the file-level key that supplied it invisibly until 2026-09-14. |
...
| `choose-gpt` | The ChatGPT-app menu, 14 actions and **no network until a row asks for it**. One Dictionary card holds every destination, a Choose from List picks a key, and the value decides the arm: containing `://` it is opened as a URL, otherwise it is run as a shortcut name. That is `choose-claude`'s open-or-run dispatch with the table held here instead of fetched, and the reason for the difference is that this menu has nothing to read. `choose-claude` pays for a `run-op` fetch because it must load the session index anyway; putting a GitHub fetch, a token injection and a JavaScript eval in front of four constants would make this one draw slowly and fail completely whenever `Inject-🎟️GitHubToken` is stale. So the rows draw offline and only `🕘 Recent sessions` reaches the network, by running `claude-session`, which needs no branch on the clipboard: with nothing branch-shaped there the op still returns `rows` from `recent` and the picker lists the twelve newest sessions. `🗂️ All sessions` sits under it as the arm that still works when the token is gone. `Out` is absent from the dictionary rather than tested, so it falls through the has-value arm and ends the chain; no card is wasted on it. It reads no Shortcut Input, so the install needs no toggle. **Two destinations are the owner's own, not this repo's guess**: `Nav-CurrentApp` has keyed a dictionary on `ChatGPT` and `Claude` since the 2026-08-13 dump, which is where the display-name spelling `get-app-route` now routes on comes from, and where `https://chatgpt.com/` is attested. **`https://chatgpt.com/codex` is not attested anywhere**: it appears in none of the fifteen dumps, whose complete AI-destination inventory is twelve URLs, and every probe of the host from a sandbox returned HTTP 403, so it is unverified until a tap says otherwise. A wrong path lands on the ChatGPT home rather than breaking the menu. **Voice mode was cut from the first build and is worth a second look**: `com.openai.chat.OpenVoiceModeIntent` is attested in the corpus (`Voice Chat`, bundle `com.openai.chat`, team `2DC432GLL2`) and is the one ChatGPT capability no URL reaches, but all seventeen corpus uses of that vendor's intents were authored on device and nothing here has ever generated a third-party App Intent card into a signed plist. Shipping an unprecedented card shape in the install that has to work is the wrong bet; it belongs in a `library-replace` once this menu is known to import. |
...
| `back-doubletap` | The 59-action dispatcher recovered from the 2026-08-22 device dump, now 67. **Not what the gesture runs**: Back Tap points at `run-backtap`, and the two are independent copies rather than a caller and a callee, so a change here reaches the phone only if the binding moves. Kept as the source of record for the arms `run-backtap` replaced with a lookup, and revised in step with it. Dispatches on the current app, then on the clipboard's type; pasted HTML goes to `Show-WebView` rather than `Show-Html`, so a quick look costs no Safari tab. Its GitHub arm calls `Show-Repo`, which the 2026-09-10 manifest says the device no longer has. |
