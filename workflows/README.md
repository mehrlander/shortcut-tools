...
| `choose-runlink` | A copied `shortcuts://run-shortcut` link in, the run offered, **or nothing**. 17 actions, and the same has-value contract `get-app-route` answers on, so a dispatcher can ask it first and still have every test below it intact. It matches the link anywhere in the text rather than anchoring to the whole clipboard, reads the target's name out of `name=` for the offer, and hands the link itself to Open URL, which is how `Fav` has run a `shortcuts://` link from inside a running shortcut since before this repo existed. Declining is the row `Out`, spelled the way `choose-backtap` spells it, because the clipboard is sticky: a link copied once would otherwise shadow every other route until something else was copied. Given no input it reads the clipboard, so it works standalone. |
